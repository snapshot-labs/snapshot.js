...
async function getEasyStakingDeposits(network, addresses, snapshot) {
  ...
  while(true) {
    ...
    const data = await subgraphRequest(EASY_STAKING_SUBGRAPH_URL[network], params);
...
async function getEasyStakingParams(network, snapshot) {
  ...
  const { commonData } = await subgraphRequest(EASY_STAKING_SUBGRAPH_URL[network], params);
  return {
    sigmoidParameters: {
...
export async function strategy(
  ...
  return Object.fromEntries(
    Object.entries(erc20Score).map(([address, balance]: any) => {
      ...
      const userDeposits = easyStakingDeposits.filter(deposit => deposit.user === address);
      userDeposits.forEach(deposit => {
        const timePassed = BigNumber.from(block.timestamp).sub(deposit.timestamp)
        const emission = calculateEmission(deposit.amount, timePassed, sigmoidParameters, totalSupplyFactor, totalSupply, totalStaked);
