...
export const calculateEmission = (deposit, timePassed, sigmoidParams, totalSupplyFactor, totalSupply, totalStaked) => {
  const d = timePassed.sub(sigmoidParams.b);
  let personalEmissionRate = ZERO;
  if (d.gt(ZERO)) {
    personalEmissionRate = sigmoidParams.a.mul(d).div(squareRoot(d.pow(TWO).add(sigmoidParams.c)));
  }
  ...
  if (totalStaked.lt(targetTotalStaked)) {
    generalEmissionRate = generalEmissionRate.mul(totalStaked).div(targetTotalStaked);
  }
  if (personalEmissionRate.isZero()) {
    generalEmissionRate = ZERO;
  }
  const emissionRate = personalEmissionRate.add(generalEmissionRate);
  const emission = deposit.mul(emissionRate).mul(timePassed).div(YEAR.mul(ONE_ETHER));
  return emission;
}
