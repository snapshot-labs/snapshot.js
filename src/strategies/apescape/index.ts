...
export async function strategy(
  ...
  const lpSupply = await call(provider, lpPairAbi, [lpPairAddress, 'totalSupply', []]);
  const lpReserves = await call(provider, lpPairAbi, [lpPairAddress, 'getReserves', []]);
  ...
  return Object.fromEntries(
    userInfoPool1.map((info1, i) => {
      ...
      return [
        addresses[i],
        parseFloat(formatUnits(sum.toString(), decimals)),
      ]
    })
  );
}
